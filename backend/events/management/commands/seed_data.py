...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # WTA Events data (matching frontend)
        wta_events = [
            {'title': "Women's Day 1", 'date': '15', 'day': 'Sun', 'month': 'FEB', 
             'time': '11:00 AM', 'event_date': date(2026, 2, 15), 'min_price': 150},
            {'title': "Women's Day 2", 'date': '16', 'day': 'Mon', 'month': 'FEB', 
             'time': '11:00 AM', 'event_date': date(2026, 2, 16), 'min_price': 150},
            {'title': "Women's Day 3", 'date': '17', 'day': 'Tue', 'month': 'FEB', 
             'time': '11:00 AM', 'event_date': date(2026, 2, 17), 'min_price': 150},
            {'title': "Women's Day 4", 'date': '18', 'day': 'Wed', 'month': 'FEB', 
             'time': '11:00 AM', 'event_date': date(2026, 2, 18), 'min_price': 150},
            {'title': "Women's Quarter-Finals", 'date': '19', 'day': 'Thu', 'month': 'FEB', 
             'time': '2:00 PM', 'event_date': date(2026, 2, 19), 'min_price': 250},
            {'title': "Women's Semi-Finals", 'date': '20', 'day': 'Fri', 'month': 'FEB', 
             'time': '1:00 PM', 'event_date': date(2026, 2, 20), 'min_price': 350},
            {'title': "Women's Finals", 'date': '21', 'day': 'Sat', 'month': 'FEB', 
             'time': '4:30 PM', 'event_date': date(2026, 2, 21), 'min_price': 500},
        ]

        # ATP Events data (matching frontend)
        atp_events = [
            {'title': "Men's Day 1", 'date': '23', 'day': 'Mon', 'month': 'FEB', 
             'time': '2:00 PM', 'event_date': date(2026, 2, 23), 'min_price': 150},
            {'title': "Men's Day 2", 'date': '24', 'day': 'Tue', 'month': 'FEB', 
             'time': '2:00 PM', 'event_date': date(2026, 2, 24), 'min_price': 150},
            {'title': "Men's Day 3", 'date': '25', 'day': 'Wed', 'month': 'FEB', 
             'time': '2:00 PM', 'event_date': date(2026, 2, 25), 'min_price': 150},
            ...
            {'title': "Men's Semi-Finals", 'date': '27', 'day': 'Fri', 'month': 'FEB', 
             'time': '1:30 PM', 'event_date': date(2026, 2, 27), 'min_price': 450},
            {'title': "Men's Finals", 'date': '28', 'day': 'Sat', 'month': 'FEB', 
             'time': '4:30 PM', 'event_date': date(2026, 2, 28), 'min_price': 750},
        ]

        # Default categories matching frontend
        default_categories = [
            {'name': 'Premium Courtside', 'price_multiplier': 8, 'color': '#1e824c', 
             'seats_total': 50, 'seats_available': 14, 'sort_order': 1},
            {'name': 'Prime A', 'price_multiplier': 4.3, 'color': '#2b5ce7', 
             'seats_total': 100, 'seats_available': 42, 'sort_order': 2},
            {'name': 'Grandstand Upper', 'price_multiplier': 1, 'color': '#86868b', 
             'seats_total': 300, 'seats_available': 156, 'sort_order': 3},
            {'name': 'Grandstand Lower', 'price_multiplier': 2.3, 'color': '#3d6ef5', 
             'seats_total': 200, 'seats_available': 88, 'sort_order': 4},
        ]

        def create_events_with_categories(tournament, events_data):
            for event_data in events_data:
                ...
                if created:
                    ...
                    base_price = event_data['min_price']
                    for cat_data in default_categories:
                        Category.objects.create(
                            event=event,
                            name=cat_data['name'],
                            price=base_price * cat_data['price_multiplier'],
        ...
        create_events_with_categories(wta_tournament, wta_events)
        
        # Create ATP events and categories
        create_events_with_categories(atp_tournament, atp_events)
