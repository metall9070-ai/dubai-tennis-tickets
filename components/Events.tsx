...
const Events: React.FC<EventsProps> = ({ onSelectEvent, initialEvents, title, subtitle }) => {
  ...
  const header = (
    <div className="mb-8 md:mb-20 text-center md:text-left">
      ...
      <p className="text-[17px] md:text-xl text-[#636366] font-normal max-w-2xl tracking-[-0.01em]">{subtitle ?? 'Discover the matches and get ready for an unforgettable experience.'}</p>
    </div>
  );
    ...
    return (
      <section id="tickets" className="pt-6 pb-12 md:pt-10 md:pb-24 bg-white text-[#1d1d1f]">
        <div className="container mx-auto px-4 sm:px-6 max-w-[980px]">
          {header}
          <div className="bg-white rounded-[24px] md:rounded-[32px] p-8 text-center text-[#636366]">
        ...
        {isTennisLayout ? (
          <>
            <div className="mb-10 md:mb-24" role="region" aria-label="WTA Tournament">
              ...
              <div className="bg-white rounded-[24px] md:rounded-[32px] overflow-hidden shadow-sm border border-black/5" role="list" aria-label="WTA events">
                {wtaEvents.length === 0 ? (
                  <div className="p-8 text-center text-[#636366]">No sessions available.</div>
                ) : (
          ...
          <div className="mb-0">
            <div className="bg-white rounded-[24px] md:rounded-[32px] overflow-hidden shadow-sm border border-black/5" role="list" aria-label="Event schedule">
              {events.length === 0 ? (
                <div className="p-8 text-center text-[#636366]">No sessions available yet. Check back soon.</div>
              ) : (
    ...
    >
      ...
      <div className="flex items-center space-x-3 sm:space-x-4 md:space-x-12">
        <div className="flex flex-col items-center justify-center min-w-[48px] sm:min-w-[56px] md:min-w-[80px] h-[48px] sm:h-[56px] md:h-[80px] bg-[#f5f5f7] rounded-xl md:rounded-2xl group-hover:bg-white transition-colors duration-300">
          <span className="text-[8px] sm:text-[9px] md:text-[11px] font-semibold text-[#636366] uppercase tracking-widest leading-none mb-0.5 sm:mb-1">{event.month}</span>
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex flex-wrap items-center gap-x-1.5 sm:gap-x-2 gap-y-0.5 mb-0.5 sm:mb-1">
             ...
             <span className="text-[11px] sm:text-[12px] md:text-[14px] font-medium text-[#636366]">{event.time}</span>
          ...
          {event.venue && (
            <p className="hidden md:block text-[14px] text-[#636366] mt-0.5">{event.venue}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-shrink-0">
        {event.isSoldOut ? (
          <span className="inline-flex items-center justify-center min-w-[44px] min-h-[44px] px-3 sm:px-4 md:px-5 py-2.5 sm:py-2 md:py-2.5 bg-[#636366] text-white text-[10px] sm:text-[11px] md:text-[12px] font-semibold uppercase tracking-wider rounded-lg">
            Sold out
          </span>
        ) : (
          <>
            <div className="flex flex-col items-end">
              {hasValidPrice ? (
                <>
                  <span className="text-[9px] sm:text-[10px] md:text-[11px] font-semibold text-[#636366] uppercase tracking-wide sm:tracking-widest">From</span>
                  <span className="text-[16px] sm:text-[18px] md:text-[22px] font-bold text-[#1d1d1f]">${event.minPrice}</span>
                </>
              ) : (
                <span className="text-[12px] sm:text-[13px] md:text-[14px] font-medium text-[#636366]">Price unavailable</span>
              )}
            </div>
