  ...
  useEffect(() => {
    ...
    async function loadEvents() {
      try {
        ...
        if (result.fallback) {
          console.error('[ATPTicketsPage] REJECTED fallback data - Django API required for prices');
        ...
        if (result.data) {
          ...
          console.log(`[ATPTicketsPage] Loaded ${atp.length} ATP events from Django API`);
        }
      } catch (err) {
        console.error('[ATPTicketsPage] Failed to load events:', err);
        if (mounted) {
          setError('Unable to load prices. Please try again.');
          setAtpEvents([]);
        }
