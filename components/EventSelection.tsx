    ...
    <div className="min-h-screen bg-white text-[#1d1d1f] flex flex-col font-sans">
      ...
      <div className="flex-1 max-w-[1200px] mx-auto px-4 md:px-6 py-6 md:py-12 w-full grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-12 border-b border-[#f5f5f7]">
        ...
        <div className="flex flex-col h-full lg:sticky lg:top-24 lg:self-start">
          ...
          {/* Cart Summary Widget */}
          {cart.length > 0 && (
            <div className="mt-auto p-6 md:p-8 bg-white text-[#1d1d1f] rounded-[24px] md:rounded-[32px] shadow-2xl sticky bottom-4 animate-fadeIn border border-black/5">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[15px] md:text-[16px] font-semibold text-[#1d1d1f]">Your Cart</span>
                <div className="flex items-center gap-2">
                  <span className="text-[13px] md:text-[14px] bg-[#f5f5f7] text-[#1d1d1f] px-3 md:px-3.5 py-1 rounded-full font-medium">
                    {cartTotalItems} {cartTotalItems === 1 ? 'ticket' : 'tickets'}
                  </span>
                  <button
                    onClick={() => setCart([])}
                    className="p-1.5 md:p-2 bg-[#f5f5f7] hover:bg-red-500/80 hover:text-white rounded-full transition-all active:scale-90"
                    title="Clear cart"
                  >
            ...
            <div className="p-5 md:p-8">
                  ...
                  <div className="bg-[#f5f5f7] rounded-[20px] md:rounded-[24px] p-4 md:p-6 mb-4 md:mb-6">
                    ...
                    <div className="flex justify-between items-baseline">
                       <span className="text-[16px] md:text-[18px] font-bold text-[#1d1d1f]">Total</span>
                       <span className="text-[22px] md:text-[26px] font-bold text-[var(--color-primary)] tracking-tight tabular-nums">${(selectedCategory.price * ticketCount).toLocaleString()}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full py-3.5 md:py-4 bg-[var(--color-primary)] text-white font-semibold rounded-2xl md:rounded-[20px] shadow-lg hover:bg-[var(--color-primary-hover)] transition-all transform active:scale-[0.98] text-[15px] md:text-[16px]"
                    >
                      Checkout
                    </button>
                    <button
                      onClick={handleContinueShopping}
                      className="w-full py-3.5 md:py-4 bg-[var(--color-primary)]/10 text-[var(--color-primary)] font-semibold rounded-2xl md:rounded-[20px] hover:bg-[var(--color-primary)]/20 transition-all transform active:scale-[0.98] text-[15px] md:text-[16px]"
                    >
                      Continue Shopping
                    </button>
                  </div>
