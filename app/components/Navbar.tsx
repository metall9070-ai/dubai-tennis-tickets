  ...
  return (
    <>
      ...
      <div
        ...
        style={{ top: siteConfig.topDisclaimer ? '32px' : '0' }}
      >
          ...
          <nav className="flex flex-col px-6 pt-2">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={(e) => handleNavItemClick(item.label, item.href, e)}
                className="text-[17px] font-semibold text-[#1d1d1f] py-3.5 border-b border-slate-100 last:border-b-0 hover:text-[var(--color-primary)] transition-colors"
              >
                {item.label}
              </Link>
            ))}
